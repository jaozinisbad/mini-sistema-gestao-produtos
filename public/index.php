...
<?php elseif ($page === 'manage' || $page === 'update'): ?><h1 class="h3 mb-3"><?= $page === 'update' ? 'Atualização com AJAX' : 'Cadastros' ?></h1><p class="text-muted">Cadastre ou edite fornecedores e produtos. A área de atualização envia alterações sem recarregar a página.</p><div id="ajax-message" role="status"></div><div class="row g-4"><div class="col-lg-5"><div class="card p-4"><h2 class="h5">Fornecedor</h2><form class="data-form" method="post"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="save_supplier"><input type="hidden" name="id" id="supplier-id"><label class="form-label">Nome</label><input class="form-control mb-2" name="name" id="supplier-name" required><label class="form-label">E-mail</label><input class="form-control mb-2" name="email" id="supplier-email" type="email"><label class="form-label">Telefone</label><input class="form-control mb-3" name="phone" id="supplier-phone"><button class="btn btn-primary">Salvar fornecedor</button><button type="reset" class="btn btn-outline-secondary">Limpar</button></form></div></div><div class="col-lg-7"><div class="card p-4"><h2 class="h5">Produto</h2><form class="data-form" method="post"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="save_product"><input type="hidden" name="id" id="product-id"><label class="form-label">Fornecedor</label><select class="form-select mb-2" name="supplier_id" id="product-supplier" required><option value="">Selecione</option><?php foreach ($suppliers as $s): ?><option value="<?= (int)$s['id'] ?>"><?= e($s['name']) ?></option><?php endforeach; ?></select><label class="form-label">Nome</label><input class="form-control mb-2" name="name" id="product-name" required><label class="form-label">Descrição</label><textarea class="form-control mb-2" name="description" id="product-description"></textarea><label class="form-label">Preço (R$)</label><input class="form-control mb-3" name="price" id="product-price" type="number" min="0" step="0.01" required><button class="btn btn-primary">Salvar produto</button><button type="reset" class="btn btn-outline-secondary">Limpar</button></form></div></div></div><div class="card p-4 mt-4"><h2 class="h5">Fornecedores</h2><div class="table-responsive"><table class="table"><thead><tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Ações</th></tr></thead><tbody><?php foreach ($suppliers as $s): ?><tr><td><?= e($s['name']) ?></td><td><?= e($s['email']) ?></td><td><?= e($s['phone']) ?></td><td><button class="btn btn-sm btn-outline-primary edit-supplier" data-record="<?= e(json_encode($s)) ?>">Editar</button> <form class="data-form d-inline" method="post" onsubmit="return confirm('Excluir fornecedor?')"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="delete_supplier"><input type="hidden" name="id" value="<?= (int)$s['id'] ?>"><button class="btn btn-sm btn-outline-danger">Excluir</button></form></td></tr><?php endforeach; ?></tbody></table></div></div><div class="card p-4 mt-4"><h2 class="h5">Produtos</h2><div class="table-responsive"><table class="table"><thead><tr><th>Nome</th><th>Fornecedor</th><th>Preço</th><th>Ações</th></tr></thead><tbody><?php foreach ($products as $p): ?><tr><td><?= e($p['name']) ?></td><td><?= e($p['supplier_name']) ?></td><td>R$ <?= number_format((float)$p['price'],2,',','.') ?></td><td><button class="btn btn-sm btn-outline-primary edit-product" data-record="<?= e(json_encode($p)) ?>">Editar</button> <form class="data-form d-inline" method="post" onsubmit="return confirm('Excluir produto?')"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="delete_product"><input type="hidden" name="id" value="<?= (int)$p['id'] ?>"><button class="btn btn-sm btn-outline-danger">Excluir</button></form></td></tr><?php endforeach; ?></tbody></table></div></div>
<div class="card p-4 mt-4"><h2 class="h5">Cesta</h2><p>Atualize a cesta sem recarregar a página.</p><?php foreach ($items as $item): ?><div class="d-flex justify-content-between border-bottom py-2"><span><?= e($item['name']) ?></span><form class="data-form" method="post"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="remove_item"><input type="hidden" name="product_id" value="<?= (int)$item['id'] ?>"><button class="btn btn-sm btn-outline-danger">Remover</button></form></div><?php endforeach; ?><a class="mt-3" href="?page=catalog">Adicionar produtos</a></div><?php elseif ($page === 'catalog'): ?><h1 class="h3">Selecionar produtos</h1><p>Escolha um ou mais produtos. Cada produto entra uma vez na cesta.</p><form method="post" id="catalog-form"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="action" value="add_items"><div class="row g-3"><?php foreach ($products as $p): ?><div class="col-md-4"><label class="card p-4 h-100"><span class="d-flex justify-content-between"><strong><?= e($p['name']) ?></strong><input class="form-check-input product-check" type="checkbox" name="product_ids[]" value="<?= (int)$p['id'] ?>"></span><span class="text-muted small mt-2"><?= e($p['supplier_name']) ?></span><span class="mt-2"><?= e($p['description']) ?></span><strong class="mt-auto pt-3">R$ <?= number_format((float)$p['price'],2,',','.') ?></strong></label></div><?php endforeach; ?></div><?php if (!$products): ?><div class="alert alert-info">Nenhum produto cadastrado ainda.</div><?php endif; ?><button class="btn btn-primary mt-4" <?= !$products ? 'disabled' : '' ?>>Adicionar à cesta</button></form>
...
<script>document.querySelectorAll('.edit-supplier').forEach(b=>b.addEventListener('click',()=>{let x=JSON.parse(b.dataset.record);for(let k of ['id','name','email','phone'])document.getElementById('supplier-'+k).value=x[k]??'';window.scrollTo({top:0,behavior:'smooth'})}));document.querySelectorAll('.edit-product').forEach(b=>b.addEventListener('click',()=>{let x=JSON.parse(b.dataset.record);for(let k of ['id','name','description','price'])document.getElementById('product-'+k).value=x[k]??'';document.getElementById('product-supplier').value=x.supplier_id;window.scrollTo({top:0,behavior:'smooth'})}));document.getElementById('catalog-form')?.addEventListener('submit',e=>{if(!document.querySelector('.product-check:checked')){e.preventDefault();alert('Selecione pelo menos um produto.')}});if(new URLSearchParams(location.search).get('page')==='update'){document.querySelectorAll('.data-form').forEach(f=>f.addEventListener('submit',async e=>{e.preventDefault();if(!f.reportValidity())return;let box=document.getElementById('ajax-message');try{let response=await fetch(location.href,{method:'POST',body:new FormData(f),headers:{'X-Requested-With':'XMLHttpRequest'}});let data=await response.json();box.className='alert '+(data.ok?'alert-success':'alert-danger');box.textContent=data.ok?'Alteração salva. Atualizando lista...':data.message;if(data.ok)setTimeout(()=>location.reload(),500)}catch{box.className='alert alert-danger';box.textContent='Falha de conexão.'}}))}</script></body></html>
